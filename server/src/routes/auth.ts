...
import { createSession, rotateTokens, invalidateSession, invalidateUserSessions, updateSessionActivity } from '../middleware/session';
import { logAudit } from '../utils/auditTrail';

const router = Router();

// CORS explícito para rotas de auth (garantir que funciona)
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const accessToken = generateAccessToken(user.id, user.email, user.role, user.tenantId);
    const refreshToken = generateRefreshToken(user.id, user.email, user.role, user.tenantId);
...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    ...
    const ipAddress = req.ip || req.headers['x-forwarded-for'] as string || req.socket.remoteAddress;
    const userAgent = req.headers['user-agent'];
    const tokens = await rotateTokens(refreshToken, ipAddress, userAgent);
